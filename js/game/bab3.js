import { matchConect } from "../gameAssets.js";
...
function initializePuzzle() {
  ...
  const puzzleData = matchConect.level1[currentPuzzleIndex];
  ...
  puzzleData.puzzles.forEach((puzzle, index) => {
    ...
    if (isSoundPuzzle) {
      ...
      targetButton.innerHTML = `<img src="${puzzle.image}" alt="Gambar ${
        index + 1
      }" />`;
    ...
    nameButton.addEventListener("click", () => {
      if (selectedTarget !== null) {
        ...
        document.getElementById("result").textContent = `Pilih ${
          isSoundPuzzle ? "Suara" : "Gambar"
        } dulu.`;
      }
    });
  ...
  const originalButtonIndices = nameButtons.map((button) =>
    parseInt(button.getAttribute("data-index"), 10)
  );
  ...
  console.log(
    "Shuffled name buttons:",
    nameButtons.map((button) => button.textContent)
  );
  ...
  nameButtons.forEach((button, index) => {
    ...
    const nameButtonsWrapper =
      buttonWrappers[originalIndex].querySelector(".name-buttons");
  ...
  console.log(
    ...
    buttonWrappers.map((wrapper) =>
      Array.from(wrapper.querySelectorAll(".name-btn")).map(
        (btn) => btn.textContent
      )
    )
  );
  ...
  document.getElementById("submit-btn").addEventListener("click", () => {
    if (document.getElementById("submit-btn").textContent === "Submit") {
      ...
      document.getElementById(
        "result"
      ).textContent = `Jawaban yang benar: ${correctMatches} dari ${puzzleData.puzzles.length}.`;
      document.getElementById("submit-btn").textContent = "Retry";
    } else {
      if (currentPuzzleIndex < matchConect.level1.length - 1) {
        currentPuzzleIndex++;
        ...
        const container = document.querySelector(".container");
        container.innerHTML = "<h1>Permainan Selesai!</h1>";
...
function drawConnection(soundIndex, name, color) {
  const targetButton = document.querySelector(
    `.sound-btn[data-index="${soundIndex}"], .image-btn[data-index="${soundIndex}"]`
  );
  ...
  const endX = nameRect.left + nameRect.width / 2 - containerRect.left;
  ...
  line.setAttribute("data-target-index", soundIndex);
...
function removeConnection(soundIndex, name) {
  ...
  const lines = svg.querySelectorAll(
    `line[data-target-index="${soundIndex}"][data-name="${name}"]`
  );
